...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default="camvid", help="dataset: cityscapes or camvid or voc or ade20k or sunrgb")
...
class Mask:
    ...
    def get_filter_similar_codebook(self, weight_torch, distance_rate, length):
        codebook = np.ones(length)
        if len(weight_torch.size()) == 4:
            similar_pruned_num = int(weight_torch.size()[0] * (1 - distance_rate))
            weight_vec = weight_torch.view(weight_torch.size()[0], -1)

            # for euclidean distance
            similar_matrix = distance.cdist(weight_vec.cpu().numpy(), weight_vec.cpu().numpy(), 'euclidean')
            # for cos similarity
            # similar_matrix = 1 - distance.cdist(weight_vec_after_norm, weight_vec_after_norm, 'cosine')
            similar_sum = np.sum(np.abs(similar_matrix), axis=0)

            # for distance similar: get the filter index with largest similarity == small distance
            similar_index_for_filter = similar_sum.argsort()[:  similar_pruned_num]
            ...
            prune_filter_index = list(np.sort(similar_index_for_filter))
        else:
            pass
    ...
    def get_filter_codebook(self, weight_torch, compress_rate, length):
        ...
        if len(weight_torch.size()) == 4:
            ...
            # for i in range(len(prune_filter_index)):
            #     prune_filter_index[i] = prune_filter_index[i] - i
    ...
    def init_mask(self, layer_rate):
        ...
        for index, item in enumerate(self.model.parameters()):
            if (index in self.mask_index):
                if args.method == 'SFP':
                    self.mat[index], self.prune_filters[index] = self.get_filter_codebook(item.data,
                                                                                          self.compress_rate[index],
                                                                                          self.model_length[index])
                elif args.method == 'FPGM':
                    self.mat[index], self.prune_filters[index] = self.get_filter_similar_codebook(item.data,
                                                                                          self.compress_rate[index],
                                                                                          self.model_length[index])
                else:
                    raise NotImplementedError("not support this pruning method")
                ...
                # # generate next conv weight mask
                # next_conv_codebook = np.ones(self.model_length[index + 4])
                # next_conv_size = self.model_size[index + 4]
                # filter_length = next_conv_size[1] * next_conv_size[2] * next_conv_size[3]
                # kernel_length = next_conv_size[2] * next_conv_size[3]
                # for i in range(next_conv_size[0]):
                #
                #     for filter_index in self.prune_filters[index]:
                #         next_conv_codebook[filter_length * i + kernel_length * filter_index:
                #                            filter_length * i + kernel_length * (filter_index + 1)] = 0
                #
                # self.mat_next_conv[index] = next_conv_codebook
                # self.mat_next_conv[index] = self.convert2tensor(self.mat_next_conv[index])
                # if args.cuda:
                #     self.mat_next_conv[index] = self.mat_next_conv[index].cuda()
...
def train_model(args):
    ...
    #ccreate log folder
    args.logdir = (args.savedir + '/' + args.dataset + '/' + "F" + "_P_" +
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        # print('Epoch %.2f: total_time %.2f%% ' % (epoch, total_time))
        ...
        logger.flush()
        print("Epoch No.: %d\t Train Loss = %.7f\t mIOU(val) = %.7f\t mIOU(test) = %.7f\t mIOU(prune)=%.7f\t lr= %.7f" % (epoch,
                                                                                    lossTr,
...
def train(args, train_loader, model, criterion, optimizer, epoch):
    ...
    # input_ = torch.randn(1, 3, 360, 480)
    # input_ = input_.cuda()
    ...
    for iteration, batch in enumerate(train_loader, 0):
        ...
        # print('Params Drop %.2f%% Flops Drop %.2f%% ' % (params, Flops))
...
def val(args, val_loader, model):
    ...
    for i, (input, label, size, name) in enumerate(val_loader):
        with torch.no_grad():
            ...
            # input = input.cuda()
            # input_var_ = input_var[0].unsqueeze(0)
            # Flops, params = profile(model, inputs=(input_var,))
            # print('=================Params Drop %.2f%% Flops Drop %.2f%% ' % (params, Flops))
