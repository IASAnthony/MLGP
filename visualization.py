...
def parse_args():
    ...
    parser.add_argument('--method', default='Taylor Small Group', type=str, help='pruning method Taylor Small or Taylor Small Group')
    parser.add_argument('--decay_rate', default='0.7', type=float, help='decay rate for do mask')
    ...
    # checkpoint and log
    parser.add_argument('--pretrain', type=str, default="/data/xmlg/Group_pruning/log/camvid/Lock_P_UNet1_Taylor Small Group_CrossEntropyLoss_1Rate_testFlag_500Epoch/best_model.pth",
...
class Mask:
    ...
    def get_taylor_small_filter_codebook(self, weight_torch, compress_rate, length, filter_grads, decay_rate, class_weights):
    ...
    def get_taylor_small_group_filter_codebook(self, weight_torch, compress_rate, length, filter_grads, decay_rate, class_weights):
        codebook = np.ones(length)
        if len(weight_torch.size()) == 4:
            filter_grads = filter_grads.data.cpu()
            filter_grads = torch.abs(filter_grads)
...
def train_model(args):
    ...
    # build the model
    model_path = '/data/xmlg/Group_pruning/log/cityscapes/Lock_P_UNet1_SFP_CrossEntropyLoss_0.2Rate_testFlag_100Epoch/best_model.pth'
    ...
    args.logdir = (args.savedir + 'visualization' + '/' + "CityScapes" + "_SFP0.2_" +
...
def val(args, val_loader, model):
    ...
    for i, (input, label, size, name) in enumerate(val_loader):
        ...
        # 将预测结果转换为灰度图像并保存
        preds_gray = preds.squeeze().cpu().numpy()  # 去除多余的维度，转换为 NumPy 数组

        # 遍历每个灰度图像，并保存到文件
        for j, gray_img in enumerate(preds_gray):
            # 构造保存路径，使用索引 i 区分不同的图像
            save_path = os.path.join(args.logdir, f"{name[j]}.png")

            # 将浮点数数组转换为整数类型，并将像素值限制在0到255之间
            gray_img = np.uint8(np.clip(gray_img, 0, 255))
